...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------------------
        space_used = len(self.contents)
        space_remaining = self.volume - space_used
        for k in range(len(additional_contents)):
            if len(self.contents + additional_contents[k]) <= self.volume:
                self.contents = self.contents + additional_contents[k]
        if space_used + len(additional_contents) <= self.volume:
            leftover = ''
        else:
            leftover = additional_contents[space_remaining:len(additional_contents)]
        return leftover
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        #######################################################################
        string = self.append_string(self.contents)
        return string
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            shrink = len(self.contents) - new_volume
            cutout = self.contents[-shrink:]
            self.contents = self.contents[:-shrink]
    ...
    def double_then_shrink(self, new_volume):
        ...
        more_remaining = self.shrink(new_volume)

        total = len(leftover) + len(more_remaining)
        return total
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        #######################################################################
        remaining = self.append_string(other_box.contents)
        other_box.contents = remaining
